...
for source in core_regression core_lifecycle core_audio; do
...
for case_name in audio-output melody-timer audio-controls audio-replay audio-state-versions; do
  timeout --kill-after=2s 30s "$root/.retrom-work/core_audio" "$case_name"
done
