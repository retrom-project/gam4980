...
with tempfile.TemporaryDirectory() as temporary:
    ...
    if json.loads((extracted / "core.json").read_text())["name"] != core:
        raise SystemExit("RETROM_CORE_ARCHIVE_INVALID")
    if (extracted / "license.txt").read_bytes() != (args.output / license_name).read_bytes():
        raise SystemExit("RETROM_CORE_LICENSE_INVALID")
license_bytes = (args.output / license_name).read_bytes()
if not license_bytes.startswith((ROOT / "COPYING").read_bytes()) or b"GNU GENERAL PUBLIC LICENSE" not in license_bytes:
    raise SystemExit("RETROM_CORE_LICENSE_INVALID")
